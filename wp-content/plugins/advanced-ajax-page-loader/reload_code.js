...
var vimeoPlayers = jQuery('div.flexslider').find('iframe');
...
function ready(player_id) {
    Froogaloop(player_id).addEvent('play', function(data) {
        jQuery('div.flexslider').flexslider("pause");
    });
    Froogaloop(player_id).addEvent('pause', function(data) {
        jQuery('div.flexslider').flexslider("play");
    });
}
jQuery('div.flexslider').flexslider({
    ...
    start: function(slider) {
        ...
        if (description) {
            jQuery('.flex-caption > .flex-caption-inner').html(description).parent().animate({
                bottom: '0px',
                left: '0px'
            }, 600, 'swing');
        }
    ...
    before: function(slider) {
        if (slider.slides.eq(slider.currentSlide).find('iframe').length !== 0) Froogaloop(slider.slides.eq(slider.currentSlide).find('iframe').attr('id')).api('pause');
        var heightShift = 0 - jQuery('.flex-caption').height() - 20;
        jQuery('.flex-caption > .flex-caption-inner').parent().animate({
            bottom: heightShift,
            left: '0px'
        }, 600, 'swing');
    },
...
jQuery(document).ready(function() {
    jQuery('div.ss_carousel ul.products').jcarousel({
        easing: 'easeInOutExpo',
        animation: 600,
        scroll: 1
    });
})
// END OF COURASEL LOAD
...
jQuery(function() {
    ...
    var insSpanEl = jQuery('span.price>ins');
    var delSpanEl = jQuery('span.price>del');
    var insPEl = jQuery('p.price>ins');
    var delPEl = jQuery('p.price>del');
   ...
   // END OF LOAD PRICE BY ROLE
   // REVIEW TAB
  jQuery('div#tab-reviews').hide();
});
